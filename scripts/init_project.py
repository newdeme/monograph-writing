...
## 3c. 关键数值登记（同一数据全书一个口径；校验脚本据此核查）

（正文首次写入关键数据（试验值/计算结果/统计量）时登记一行：
`- 数值串（正文精确写法，含单位，如 7.5 h）｜含义｜首现小节｜关联图表（图X-Y，无则填 -）`。
后续章节/图表/小结引用同一数据须与登记串逐字一致；校验脚本核对首现可溯与关联数据图；
语义级变体（7.5 h vs 7小时30分）终检时人工过一遍。）

- （暂无）

## 4. 已核实文献缓存（题录核对通过，可直接引用）

（格式：`- 作者. 题名[J]. 刊名, 年, 卷(期): 页码. ｜ 已核：日期＋核对位置 ｜ 核验状态：已核实/字段不符/未找到/须人工`。
新核实的当日登记，节省后续批次重查；核验四态说明见技能 references/evidence-corpus.md §7b——
字段不符与未找到的禁入正文，须人工的请作者裁定；登记前按 DOI/ISBN（或题名＋首作者）查重。）
...
## 4c. 检索记录（每次为写作批次做的文献检索；留痕可审计）

（格式：`- 日期｜渠道（知识库名/Zotero 集合/语料文件夹）｜检索式｜命中数｜采纳数`。
后续批次可复用检索式增量更新，也避免"以为查过其实没查到"。）
