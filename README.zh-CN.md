...
| 成稿语言质量参差、有 AI 腔 | 润色批次：三轮润色法（论证/图表细节/术语语言）＋保守修改原则（含义性改动须作者裁定）＋中文去 AI 腔清单 |
...
| `scripts/validate_manuscript.py` | 校验命名/结构/字数/引用编号/图表编号/关键数值一致性 | 每批次收尾必跑 |
...
- **第三方评测与安全检测**：SkillHub TRACE 评测 **4.7/5 · 优秀**（可信度 5.0；报告基于 v1.4.0 初版）；腾讯安全云鼎实验室 Skills 安全评估 **100/100 · 可信**，腾讯安全威胁情报中心结论「**安全**」（安全报告基于 v1.4.0 包，2026-09-07）。
...
**Q：我的书稿会被上传或收集吗？**
不会。本技能的五个脚本全部在你的电脑本地运行，不联网、无遥测、不收集任何数据；书稿与台账都保存在你自己的磁盘上。文献检索经由你自己的 AI 助手与知识库完成，由你掌控（详见"数据隐私与技术特性"一节）。
...
**Q：AI 能帮我润色吗？会不会越润越走样？**
能，且有专门流程（润色批次）：三轮推进——第一轮核对论证完整、第二轮核对图表与数值细节、第三轮统一术语时态并清除 AI 腔（「值得注意的是」「具有重要意义」这类空洞表达按清单处理）。**保守修改原则**保证不走样：错字病句直接修；凡可能改变科学含义的改动（相关性改成因果性、删限定条件、增删数字）一律保留原句、列出建议交你裁定，不直接替换。详见 `references/polish-workflow.md`。

**Q：同一数据在正文、图表、章末小结里说法不一怎么办？**
关键数据在台账「关键数值登记」段落登记（数值｜含义｜首现小节｜关联图表），后续各处引用同一数据须与登记串逐字一致，校验脚本发现不一致即报「关键数值不一致」WARN——数字口径由脚本把关，不靠人肉通读。
